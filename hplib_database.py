...
def NormalizeKeymarkData(filename):
    ...
    for model in Models:
        data_key = pd.read_csv(r'output/'+filename) #read Dataframe of all models
        data=data_key.loc[((data_key['Model']==model) & (data_key['T_out [°C]']==52))]#only use data of model and ref point -7/52
...
def ReduceToUnique():
    ...
    for values in unique_values:
        modelnames=df.loc[df['p3_P_el [-]']==values,['Model']].values.tolist()
        try:
            Models.append(modelnames[0][0])
        except:
            continue
    new_df=pd.DataFrame()
