...
def import_heating_data():
                ...
                for lines in contents:
                    ...
                    if lines.startswith("Prated"):
                        ...
                        if contents[i] == "\n":  # no content
                            ...
                            if contents[i + 2].startswith(
                                "COP"
                            ):  # for PDF without medium heat
                                continue
                            if contents[i + 2].startswith(
                                "Disclaimer"
                            ):  # for PDF without medium heat
                            ...
                            if contents[i + 2].startswith("EHPA"):  # End of page
                                if contents[i + 8].startswith(
                                    "COP"
                                ):  # end of page plus no medium heat
                                    continue
    ...
    df["PSB [W]"] = df["PSB [W]"].where(
        df["PSB [W]"] > df["Poff [W]"], df["Poff [W]"]
    )  # Poff should not be bigger than PSB
...
def reduce_heating_data(filename, climate):
    ...
    for model in Models:
        ...
        if (
            Modeldf.shape[0] != 8
        ):  # Models with more ar less than 8 datapoints are deleted
...
def normalize_heating_data(filename):
    ...
    for model in Models:
        data_key = pd.read_csv(r"../output/" + filename)  # read Dataframe of all models
        data = data_key.loc[
            ((data_key["Model"] == model) & (data_key["T_out [°C]"] == 52))
        ]  # only use data of model and ref point -7/52
        ...
        data_key.loc[:, ["P_th_n"]] = (
            data_key["P_th [W]"] / Pth_ref
        )  # get normalized Value P_th_n
        data_key.loc[:, ["P_el_n"]] = (
            data_key["P_el [W]"] / Pel_ref
        )  # get normalized Value P_el_n
    ...
    new_df.to_csv(
        r"../output/" + filename[:-4] + "_normalized.csv", encoding="utf-8", index=False
    )
...
def identify_subtypes(filename):
    ...
    filt1 = (data_key["Type"] == "Outdoor Air/Water") & (
        data_key["Subtype"] == "Regulated"
    )
    data_key.loc[filt1, "Group"] = 1
    filt1 = (data_key["Type"] == "Exhaust Air/Water") & (
        data_key["Subtype"] == "Regulated"
    )
    ...
    data_key.to_csv(
        r"../output/" + filename[:-4] + "_subtypes.csv", encoding="utf-8", index=False
    )


def fit_simple(w, x, y, z):
    p0 = [0.1, 0.001, 0.1, 1.0]  # starting values
    a = (w, x, y, z)
...
def calculate_heating_parameters(filename):
    ...
    for model in Models:
        ...
        if (
            group > 1 and group != 4
        ):  # give another point at different Temperature of Brine/Water
        ...
        if group == 1 or group == 2 or group == 3:
            ...
            P_el_n_para_key = fit_simple(
                data["T_in"], data["T_out"], data["T_amb"], data["P_el_n"]
            )
            P_th_n_para_key = fit_simple(
                data_key["T_in"],
                data_key["T_out"],
                data_key["T_amb"],
                data_key["P_th_n"],
            )
            COP_para_key = fit_simple(
                data_key["T_in"], data_key["T_out"], data_key["T_amb"], data_key["COP"]
            )
        else:
            P_el_n_para_key = fit_simple(
                data_key["T_in"],
...
def validation_relative_error_heating():
    ...
    df.to_csv(
        "../output/database_heating_average_normalized_subtypes_validation.csv",
        encoding="utf-8",
        index=False,
    )
...
def validation_mape_heating():
    ...
    for model in Models:
        ...
        mape_pth.append(
            (
                ((df_model["P_th [W]"] - df_model["P_th_sim"]).abs())
                / df_model["P_th [W]"]
                * 100
            ).mean()
        )
        mape_pel.append(
            (
                ((df_model["P_el [W]"] - df_model["P_el_sim"]).abs())
                / df_model["P_el [W]"]
                * 100
            ).mean()
        )
        mape_cop.append(
            (
                ((df_model["COP"] - df_model["COP_sim"]).abs()) / df_model["COP"] * 100
            ).mean()
        )
...
def add_generic():
    ...
    for group in Groups:
        ...
        data_key.loc[len(data_key.index)] = [
            "Generic",
            "Generic",
            "",
            Type,
...
def reduce_cooling_data():
    ...
    df = df_cool.merge(
        df_heat, on="Model", how="left"
    )  # merge with the ones from heating to get Group Number
    df = df.iloc[:, :16]
    ...
    df = df.rename(
        columns={"Manufacturer_x": "Manufacturer", "T_out [°C]_x": "T_out [°C]"}
    )
...
def normalize_and_add_cooling_data():
    ...
    for model in Models:
        ...
        if len(data_key) == 4:
            data_key1 = data_key.loc[data_key["Model"] == model]
            data_key1["T_out [°C]"] = (
                data_key1["T_out [°C]"] + 11
            )  # the following values are based on 3 heatpumps, which have those values in the keymark
            data_key1.loc[data_key1["T_outside [°C]"] == 35, "P_el [W]"] = (
                data_key1.loc[data_key1["T_outside [°C]"] == 35, "P_el [W]"] * 0.85
            )
            data_key1.loc[data_key1["T_outside [°C]"] == 30, "P_el [W]"] = (
                data_key1.loc[data_key1["T_outside [°C]"] == 30, "P_el [W]"] * 0.82
            )
            data_key1.loc[data_key1["T_outside [°C]"] == 25, "P_el [W]"] = (
                data_key1.loc[data_key1["T_outside [°C]"] == 25, "P_el [W]"] * 0.77
            )
            data_key1.loc[data_key1["T_outside [°C]"] == 20, "P_el [W]"] = (
                data_key1.loc[data_key1["T_outside [°C]"] == 20, "P_el [W]"] * 0.63
            )
            data_key1.loc[data_key1["T_outside [°C]"] == 35, "EER"] = (
                data_key1.loc[data_key1["T_outside [°C]"] == 35, "EER"] * 1.21
            )
            data_key1.loc[data_key1["T_outside [°C]"] == 30, "EER"] = (
                data_key1.loc[data_key1["T_outside [°C]"] == 30, "EER"] * 1.21
            )
            data_key1.loc[data_key1["T_outside [°C]"] == 25, "EER"] = (
                data_key1.loc[data_key1["T_outside [°C]"] == 25, "EER"] * 1.20
            )
            data_key1.loc[data_key1["T_outside [°C]"] == 20, "EER"] = (
                data_key1.loc[data_key1["T_outside [°C]"] == 20, "EER"] * 0.95
            )
            data_key1["Pdc [W]"] = data_key1["P_el [W]"] * data_key1["EER"]
            data_key = pd.concat([data_key, data_key1])
        df_ref_pdc = data_key.loc[
            (data_key["T_outside [°C]"] == 35) & (data_key["T_out [°C]"] == 7),
            "Pdc [W]",
        ].values[0]
        data_key["Pdc_n"] = data_key["Pdc [W]"] / df_ref_pdc
        df_ref_p_el = data_key.loc[
            (data_key["T_outside [°C]"] == 35) & (data_key["T_out [°C]"] == 7),
            "P_el [W]",
        ].values[0]
        ...
        new_df = pd.concat([new_df, data_key])  # merge new Dataframe with old one
    new_df.to_csv(
        "../output/database_cooling_reduced_normalized.csv",
        encoding="utf-8",
        index=False,
    )
...
def calculate_cooling_parameters():
    ...
    for model in Models:
        ...
        Pdc_n_para_key = fit_simple(
            data_key["T_in"], data_key["T_out"], data_key["T_amb"], data_key["Pdc_n"]
        )
        EER_para_key = fit_simple(
            data_key["T_in"], data_key["T_out"], data_key["T_amb"], data_key["EER"]
        )
    ...
    para.rename(
        columns={
            "P_el_cooling_ref": "P_el_c_ref [W]",
            "P_el_ref [W]": "P_el_h_ref [W]",
            "Pdc_ref": "P_th_c_ref [W]",
            "P_th_ref [W]": "P_th_h_ref [W]",
        },
        inplace=True,
    )
    para = para[
...
def validation_mape_cooling():
    ...
    for model in Models:
        ...
        mape_pdc.append(
            (
                ((df_model["Pdc [W]"] - df_model["Pdc_sim"]).abs())
                / df_model["Pdc [W]"]
                * 100
            ).mean()
        )
        ...
        mape_eer.append(
            (
                ((df_model["EER"] - df_model["EER_sim"]).abs()) / df_model["EER"] * 100
            ).mean()
        )
