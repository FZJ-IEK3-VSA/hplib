...
def get_parameters(
    model: str, group_id: int = 0, t_in: int = 0, t_out: int = 0, p_th: int = 0
) -> pd.DataFrame:
    """
        ...
        if group_id == 1:
            try:
                ...
                parameters["P_el_c_ref [W]"] = (
                    p_el_ref * 0.6852
                )  # average value from real Heatpumps (P_el35/7 to P_el-7/52)
...
def fit_func_p_th_ref(
    p_th: int, t_in: int, t_out: int, group_id: int, p_th_set_point: int
) -> int:
    """
...
def simulate(
    ...
    if group_id == 1 or group_id == 4:
        ...
        pass
    if (
        type(t_in) == pd.core.series.Series
        or type(t_in_secondary) == pd.core.series.Series
        or type(t_amb) == pd.core.series.Series
    ):  # for handling pandas.Series
        try:
            df = t_in.to_frame()
        ...
        if group_id == 1 or group_id == 2 or group_id == 3:
            df.loc[df["Modus"] == 1, "COP"] = (
                p1_cop * t_in + p2_cop * df["T_out"] + p3_cop + p4_cop * t_amb
            )
            df.loc[df["Modus"] == 1, "P_el"] = (
                p1_p_el * t_in + p2_p_el * df["T_out"] + p3_p_el + p4_p_el * t_amb
            ) * p_el_ref  # this is the first calculated value for P_el
            if (
                group_id == 1
            ):  # with regulated heatpumps the electrical power can get too low. We defined a minimum value at 25% from the point at -7/output temperature.
                df.loc[df["Modus"] == 2, "EER"] = (
                    p1_eer * t_in + p2_eer * df["T_out"] + p3_eer + p4_eer * t_amb
                )
                df.loc[df["Modus"] == 2, "P_el"] = (
                    p5_p_el * t_in + p6_p_el * df["T_out"] + p7_p_el + p8_p_el * t_amb
                ) * p_el_col_ref
                df.loc[
                    (df["Modus"] == 2) & (df["T_in"] < 25), "P_el"
                ] = p_el_col_ref * (
                    p5_p_el * 25 + p6_p_el * df["T_out"] + p7_p_el + p8_p_el * 25
                )
            ...
            df.loc[
                (df["Modus"] == 1) & (df["COP"] < 1), "P_el"
            ] = p_th_ref  # if COP is too low the electeric heating element is used in simulation
        ...
        if modus == 1:
            t_out = (
                t_in_secondary + DELTA_T
            )  # Inlet temperature is supposed to be heated up by 5 K
            EER = numpy.nan
        ...
        if group_id == 1 or group_id == 2 or group_id == 3:
            if modus == 1:
                ...
                P_el = (
                    p1_p_el * t_in + p2_p_el * t_out + p3_p_el + p4_p_el * t_amb
                ) * p_el_ref
                if group_id == 1:
                    t_in = -7
                    t_amb = t_in
                ...
                if t_in < 25:
                    ...
                    t_amb = t_in
                P_el = (
                    p5_p_el * t_in + p6_p_el * t_out + p7_p_el + p8_p_el * t_amb
                ) * p_el_col_ref
                if P_el < 0:
                    EER = numpy.nan
                    P_el = numpy.nan
...
class HeatPump:
    ...
    def simulate(
        self, t_in_primary: float, t_in_secondary: float, t_amb: float, modus: int = 1
    ) -> dict:
        """
        Performs the simulation of the heat pump model.
