...
def load_database() -> pd.DataFrame:
    ...
    """
    return pd.read_csv('hplib_database.csv')


def get_parameters(model: str, group_id: int = 0, t_in: int = 0, t_out: int = 0, p_th: int = 0) -> dict:
    """
...
def get_parameters_fit(model: str, group_id: int = 0, p_th: int = 0) -> pd.DataFrame:
    ...
    if model == 'Generic':
        ...
        if group_id in {1, 4}:
            t_in_fix = -7
        # Brine/Water
        elif group_id in {2, 5}:
            t_in_fix = 0
        # Water/Water
        elif group_id in {3, 6}:
            t_in_fix = 10
...
def fit_func_p_th_ref(p_th: int, t_in: int, t_out: int, group_id: int, p_th_set_point: int) -> int:
    ...
    p_th_diff : numeric
        Thermal output power. [W]
    """
    t_amb = t_in if group_id in {1, 4} else -7
    ...
    return p_th_calc - p_th_set_point
...
def simulate(t_in_primary: any, t_in_secondary: any, parameters: pd.DataFrame,
    ...
    if group_id in {1, 4}:
        t_amb = t_in
    if any(isinstance(i, pd.core.series.Series) for i in (t_in, t_out, t_amb)):  # for handling pandas.Series
        # The previous procedure overrides the error handling for every occurring error.
        # For example, if the renaming fails, this error will not be detected anymore.
        # An alternative code could be the following:
        # Still, this solution is ambiguous in the case if more than one variable is an instance of the pandas series.

        if isinstance(t_in, pd.core.series.Series):
            df = t_in.to_frame()
            df.rename(columns={t_in.name: 'T_in'}, inplace=True)
            df['T_out'] = t_out
            df['T_amb'] = t_amb

        if isinstance(t_out, pd.core.series.Series):
            df = t_out.to_frame()
            df.rename(columns={t_out.name: 'T_out'}, inplace=True)
            df['T_in'] = t_in
            df['T_amb'] = t_amb

        if isinstance(t_amb, pd.core.series.Series):
            df = t_amb.to_frame()
        ...
        if group_id in {1, 2, 3}:
            ...
            df['P_el'] = (p1_p_el * t_in
                          + p2_p_el * t_out
                          + p3_p_el
                          + p4_p_el * t_amb) * p_el_ref  # this is the first calculated value for P_el
            # with regulated heat pumps the electrical power can get too low. We defined a minimum
            # value at 25% from the point at -7/output temperature.
            if group_id == 1:
                df.loc[:, 't_in'] = -7
                df.loc[:, 't_amb'] = -7
            if group_id == 2:
            ...
            p_el_25 = 0.25 * p_el_ref * (p1_p_el * df['t_in']
                                         + p2_p_el * df['T_out']
                                         + p3_p_el
                                         + p4_p_el * df['t_amb'])

            df.loc[df['P_el'] < p_el_25, 'P_el'] = p_el_25
            df['P_th'] = df['P_el'] * df['COP']
            # if COP is too low the electeric heating element is used in simulation
            ...
            df['P_th'] = df['P_el'] * df['COP']
            df.loc[df['COP'] < 1, 'P_el'] = p_th_ref
            # if COP is too low the electric heating element is used in simulation
            df.loc[df['COP'] < 1, 'P_th'] = p_th_ref
            df.loc[df['COP'] < 1, 'COP'] = 1
            df['m_dot'] = df['P_th'] / (DELTA_T * CP)
        ...
        if group_id in {1, 2, 3}:
            ...
            p_el_25 = 0.25 * p_el_ref * (p1_p_el * t_in + p2_p_el * t_out + p3_p_el + p4_p_el * t_amb)
            if P_el < p_el_25:
                P_el = p_el_25
            P_th = P_el * COP
            if COP <= 1:
...
class HeatPump:
    ...
    def simulate(self, t_in_primary: Union[float, npt.ArrayLike],
        ...
        if self.group_id in {1, 2, 3}:
            ...
            if self.group_id == 1:
                t_in = np.full_like(t_in, -7) if isinstance(t_in, np.ndarray) else -7
                t_amb = t_in

            elif self.group_id == 2:
                # 25% of Pel @ -7°C T_amb = T_in
                t_amb = np.full_like(t_amb, -7) if isinstance(t_amb, np.ndarray) else -7
        ...
        if self.group_id in {1, 2, 3, 4, 5, 6}:
            ...
            if isinstance(cop, np.ndarray):
                ...
                p_el = self.p_th_ref  # In this case the previous value will be overwritten for group_id 1 to 3.
                p_th = self.p_th_ref
            elif cop <= 1:
                cop = 1
                p_el = self.p_th_ref
